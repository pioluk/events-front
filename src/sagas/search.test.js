...
describe('search saga', () => {
  it('searchEventsFlow success', () => {
    const generator = searchEventsFlow()

    expect(generator.next().value).toEqual(take(SEARCH_EVENTS))

    const page = 1
    const query = 'lorem'
    expect(generator.next({ page, query }).value).toEqual(call(searchEvents, page, query))
    ...
    expect(generator.next({ count, events }).value).toEqual(put(searchEventsSuccess(count, events)))

    expect(generator.next().value).toEqual(take(SEARCH_EVENTS))
  })

  it('searchEvents failure', () => {
    const generator = searchEventsFlow()
    ...
    const error = new Error('Unable search events.')
    expect(generator.throw(error).value).toEqual(put(fetchEventsFailure(error)))
