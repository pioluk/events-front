...
describe('search reducer', () => {
  it('should handle SEARCH_EVENTS', () => {
    const page = 1
    const query = 'lorem'
    const state = reducer(undefined, actions.searchEvents(page, query))
    expect(state).toMatchSnapshot()
  })

  it('should handle SEARCH_EVENTS_RESET', () => {
    const initialState = {
      count: 2,
      error: null,
      events: [{ id: 1 }, { id: 2 }],
    ...
    const state = reducer(initialState, actions.searchEventsReset())
    expect(state).toMatchSnapshot()
  })

  it('should handle SEARCH_EVENTS_SUCCESS', () => {
    ...
    const state = reducer(undefined, actions.searchEventsSuccess(totalCount, events))
    expect(state).toMatchSnapshot()
  })
})
